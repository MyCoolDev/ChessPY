...
from GameStates.InOfflineGame import InOfflineGame
...
class MainMenu(BaseState):
    ...
    def __init_vars(self, screen: pygame.Surface = None, *args, **kwargs):
        ...
        self.start_offline_button = Button(pygame.Vector2(340, 50), pygame.Vector2(screen.get_width() / 2, self.start_button.position.y + self.start_button.size.y + 50), (122, 122, 122), "Play Offline", "Open Sans", 20, (255, 255, 255), border_radius=6)
        self.exit_button = Button(pygame.Vector2(340, 50), pygame.Vector2(screen.get_width() / 2, self.start_offline_button.position.y + self.start_offline_button.size.y + 50), (255, 74, 74), "Exit", "Open Sans", 20, (255, 255, 255), border_radius=6)
    ...
    def update(self, dt: float, events: list, *args, **kwargs):
        ...
        if self.start_offline_button.update(dt, events):
            self.state_manager.insert_state(InOfflineGame(self.state_manager, self.screen, self))
            self.state_manager.remove_state(1)
        if self.exit_button.update(dt, events):
            pygame.quit()

        # chess theme changers
    ...
    def render(self, screen: pygame.Surface, *args, **kwargs):
        ...
        self.start_offline_button.render(screen)
